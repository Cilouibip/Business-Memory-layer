...
export type ExtractionCategory = 'contenu' | 'offre' | 'client' | 'strategie' | 'metrique' | 'process' | 'autre';
export type ExtractionPromptDocument = { source_type: string; raw_payload: Record<string, unknown> };
...
export function buildPrompt(template: string, rawDoc: ExtractionPromptDocument, summary: string): string {
  return template
...
export function getPromptForCategory(category: ExtractionCategory): string {
  switch (category) {
...
export function getSchemaForCategory(category: ExtractionCategory) {
  switch (category) {
